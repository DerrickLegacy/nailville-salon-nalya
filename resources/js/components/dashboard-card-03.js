...
const dashboardCard03 = () => {
    ...
    fetch('/json-dashboard-net') // your route returning today vs yesterday data
        .then(res => res.json())
        .then(result => {
            // console.log("Results dash-03:", result.labels);

            // Map datasets directly from API
            const todayData = result.datasets.find(ds => ds.label === 'Today Net')?.data || [];
            const yesterdayData = result.datasets.find(ds => ds.label === 'Yesterday Net')?.data || [];
